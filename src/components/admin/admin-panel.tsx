      ...
      <Tabs value={tab} onValueChange={setTab}>
        <TabsList className="grid w-full grid-cols-2 sm:grid-cols-5">
          ...
          <TabsTrigger value="audit" className="gap-1.5">
            <Pulse weight="regular" className="h-3.5 w-3.5" />
          ...
          {stats ? (
            <>
              <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
                <StatCard icon={<Users weight="regular" className="h-5 w-5 text-accent" />} value={String(stats.users.total)} label="Jami foydalanuvchilar" sub={`${stats.recent.signups} yangi (7 kun)`} />
                <StatCard icon={<Briefcase weight="regular" className="h-5 w-5 text-trust-verified" />} value={String(stats.advocates.verified)} label="Tasdiqlangan advokatlar" sub={`${stats.advocates.pending} kutilmoqda`} />
                <StatCard icon={<FileText weight="regular" className="h-5 w-5 text-trust-premium" />} value={String(stats.documents.total)} label="Hujjat shablonlari" sub={`${stats.documents.drafts} draf`} />
                <StatCard icon={<ChatCircle className="h-5 w-5 text-accent" />} value={String(stats.messaging.messages)} label="Jami xabarlar" sub={`${stats.recent.messages} yangi (7 kun)`} />
              </div>
            ...
            <div className="flex justify-center py-8"><Spinner weight="regular" className="h-6 w-6 animate-spin" /></div>
          )}
        </TabsContent>
                ...
                <Card key={a.id} className="border-border p-5">
                  <div className="flex flex-col gap-4 sm:flex-row sm:items-start">
                    <div className="min-w-0 flex-1">
                      <div className="mb-2 flex flex-wrap items-center gap-2">
                        ...
                        <Badge className="bg-amber-100 text-amber-700 hover:bg-amber-100 text-[11px] gap-1">
                          <Clock weight="regular" className="h-2.5 w-2.5" />
                          Kutilmoqda
                        </Badge>
                      </div>
                    ...
                    <div className="flex shrink-0 flex-col gap-2">
                      ...
                      >
                        {processing === a.id ? <Spinner weight="regular" className="h-3.5 w-3.5 animate-spin" /> : <CheckCircle weight="regular" className="h-3.5 w-3.5" />}
                        Tasdiqlash
                      </Button>
                      <Button
                        size="sm"
