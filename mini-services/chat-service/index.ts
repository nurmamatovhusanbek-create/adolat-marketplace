...
io.on("connection", (socket: Socket) => {
  ...
  socket.on("conversation:join", async (data: { conversationId: string }, ack?: (response: unknown) => void) => {
    try {
  ...
  socket.on("message:send", async (data: { conversationId: string; receiverId: string; content: string; attachments?: any[] }, ack?: (response: unknown) => void) => {
    try {
