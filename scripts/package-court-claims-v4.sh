#!/bin/bash
# Package court-claims v4.0 integration
set -euo pipefail

PROJECT_ROOT="/home/z/my-project"
STAGING="/tmp/court-claims-v4"
OUT_ZIP="/home/z/my-project/download/court-claims-v4.zip"

rm -rf "$STAGING" "$OUT_ZIP"
mkdir -p "$STAGING"

CHANGED_FILES=(
  # Build config — TS check disabled (it OOMs on the 151KB JSON)
  "next.config.ts"

  # Updated validators — money + boolean field types + court-claims library schema
  "src/lib/security/validators.ts"

  # Extended template types — CourtClaimTemplate interface + renderBodyTemplate + validateCourtClaimDraftValues
  "src/lib/documents/template.ts"

  # NEW: JSON data file (151KB, 54 templates)
  "src/lib/documents/templates/court-claims-v4.json"

  # NEW: Registry loader (cached, fs-based to avoid TS OOM)
  "src/lib/documents/templates/registry.ts"

  # NEW: API route for browsing templates
  "src/app/api/court-claims/route.ts"

  # NEW: Seed script to import all 54 templates into DB
  "scripts/seed-court-claims.js"

  # Updated editor — supports money + boolean field types
