...
### Interface Overhaul — "Registry, Refined"

Execution of the Interface Overhaul Brief: consolidate the existing Registry
design system (do **not** add an 8th palette layer on top), delete confirmed
dead work, and execute the current direction to a premium standard.

### Deleted — cleanup pass (§9.1)
- **12 orphaned root-level `.tsx` files** — stale duplicates left by a previous
  packaging script (`advocate-card.tsx`, `badge.tsx`, `category-card.tsx`,
  `document-card.tsx`, `dynamic-icon.tsx`, `how-it-works-page.tsx`, `input.tsx`,
  `pagination.tsx`, `request-card.tsx`, `sidebar.tsx`, `testimonial-card.tsx`,
  `unified-search.tsx`). Verified zero import sites in `src/` before deletion.
- **`generate-route.ts`** at repo root — orphan duplicate of
  `src/app/api/documents/[slug]/generate/route.ts`.
- Most §1 orphans from the brief were already gone in this repo state
  (`marketplace/hero.tsx`, `featured-advocates.tsx`, `testimonials.tsx`,
  `how-it-works.tsx`, `category-grid.tsx`, `advocate-cta.tsx`,
  `navigation-dock.tsx`, `mobile-nav-sheet.tsx`, `layout/page-layout.tsx`,
  `ui/button.tsx`, `ui/card.tsx`, `ui/toast.tsx`, `ui/toaster.tsx`,
  `hooks/use-toast.ts`). Re-verified with fresh grep — confirmed absent.

### Added — tokens & primitives (§4, §5)
- `globals.css`: `.font-display` / `.font-body` / `.font-mono` explicit utilities
  so `font-display` works as a class on non-heading elements (IBM Plex Sans via
  next/font's `--font-display` var). Previously `font-display` was used in 18
  files but was not a valid Tailwind utility — silently no-op on spans/divs.
- `globals.css`: `.bg-seal-tint` / `.bg-stamp-tint` / `.border-emphasis` utilities
  mapping the additive §4.1 tokens to usable classes.
- `globals.css`: `fadeIn` + `riseOut` keyframes and exit animations for
  `[data-state="closed"][data-slot="dialog-content"]` / `dialog-overlay` so
  modals animate out cleanly instead of hard-unmounting (§5). Mirrors `riseIn`
  timing/easing — no new feel invented.
- `ui/badge.tsx`: extended with `tone` (`neutral` / `brand` / `success` /
  `warning` / `danger` / `info`) and `size` (`sm` / `md` / `lg`) variants so
  card components can request consistent small badges without hand-rolled
  styles. Tones map to the Registry token system (`stamp-tint`, `seal-tint`).

### Changed — shared cards (§6, §7)
- **`cards/request-card.tsx`** — added systematic `REQ·XXXX` reference code
  (top-right of header, always) per §3.3; added `approved` field that renders
  the Seal next to the title; added `statusLabel` field for status badges;
  budget pill now uses `bg-stamp-tint`/`text-success` instead of ad hoc green.
- **`cards/document-card.tsx`** — renders the Seal next to the title when a
  document cites a legal basis (`legalBasisUz`) or explicitly opts in via
  `verified` (§3.2 — "generated documents" get the Seal).
- **`marketplace/document-listing.tsx`** — deleted the inline hand-rolled
  `DocumentCard` function (was 75 lines of duplicated markup); now imports and
  uses the shared `DocumentCard` from `cards/`. Page wrapper migrated to
  `<Section>` + `<Container>` primitives; results grid migrated to `<Grid>`.
- **`marketplace/popular-documents.tsx`** — replaced hand-rolled card markup
  (110 lines) with shared `DocumentCard`; replaced `animate-pulse` skeleton
  with a shaped `skeleton-shimmer` placeholder (§5); wired `useInView` for
  scroll-triggered reveal; added designed empty state.
- **`marketplace/recent-requests.tsx`** — replaced hand-rolled card markup
  with shared `RequestCard`; replaced `animate-pulse` with `skeleton-shimmer`;
  wired `useInView`; added designed empty state.
- **`marketplace/requests-page.tsx`** — replaced hand-rolled card markup with
  shared `RequestCard`; page wrapper migrated to `<Section>` + `<Container>`;
  added "clear filters" action to the empty state.
- **`primitives/typography.tsx`** — fixed `font="display"` mapping (was
  emitting `font-sans` i.e. Inter — the body font — instead of the display
  font; bug since the Playfair removal). Updated stale doc comment.
- **`components/search/unified-search.tsx`** — fixed `<Button inputSize="lg">`
  → `<Button size="lg">` (pre-existing type error; `inputSize` was never a
  valid Button prop).

### Changed — motion (§5)
- `home/testimonials-section.tsx`, `home/document-categories-section.tsx`,
  `home/featured-advocates-section.tsx` — replaced hardcoded `reveal-stagger
  in-view` (which fired on mount, missing below-the-fold users) with the
  shared `useInView` hook so the reveal triggers on scroll-into-view. Fires
  once; respects `prefers-reduced-motion`.

### Changed — token color cleanup (§4.1)
- Replaced **all** stale `emerald-*` / `red-*` / `gray-*` colors with token
  colors across 9 files (`auth-modal.tsx`, `dashboard.tsx`,
  `document-editor.tsx`, `advocate-detail-modal.tsx`, `how-it-works-page.tsx`,
  `for-advocates-page.tsx`, `post-request-modal.tsx`, `document-detail-modal.tsx`,
  `advocate-listing.tsx`). Every color now resolves through the Registry token
  system (`--success`/`--stamp-green`, `--destructive`/`--seal`, `--muted`).
  Zero stale colors remain in `src/` (verified by grep).

### QA — §9.5 final pass
- `rg "(p|m|gap)-\[[0-9]+px\]"` outside `ui/` shadcn defaults → **0 matches**.
- `rg "@/components/ui/button|@/components/ui/card"` → **0 matches**.
- `rg "framer-motion"` → only `src/app/page.tsx` (the one place §5 calls for).
- `rg "ui/toast|use-toast"` → **0 matches**.
- `rg "emerald-|bg-red-|text-red-"` in `src/` → **0 matches**.
- `tsc --noEmit` → 2 pre-existing errors in `api/documents/[slug]/generate/route.ts`
  (Buffer→BodyInit type), unchanged by this pass and out of scope per brief
  (API/business logic not in scope).

### Notes
- The ~26 unused shadcn `ui/` primitives (accordion, alert-dialog, breadcrumb,
  calendar, etc.) are intentionally left in place per §9.1 — low-risk
  scaffolding, not urgent. Future pass can prune them.
- `framer-motion` is now genuinely load-bearing (view transitions in
  `page.tsx` per §5). Modals use CSS exit animations (added this pass) rather
  than `AnimatePresence` — this keeps Radix Dialog's API and avoids
  redundant JS orchestration for something CSS handles cleanly.
